/* 
 * Provide an simpler and easier to understand interface to the System V semaphore
 * system call. There are 7 routines available to the user:
 *
 ...
 *
 * We create and use a 3-member set for the requested semphore.
 * The first member, [0], is the actual semaphore value, and the second member
 * [1], is a counter used to know when all process have finished with the semphore.
 * The counter is initialized to a large number.
 * decremented on every create or open and incremented on every close.
 * This way we can use the "adjust" feature provided by System V so that any process
 * that exit's without calling sem_close() is accounted for.It doesn't help us if the last 
 * process does this (as we have no way of getting control to remove the semphore) but it 
 * will work if any process other than the last does an exit (interntional or unintentinal).
 *
 * The third member, [2], of the semaphore set is used as a lock varibale to avaoid any race conditions
 * in the sem_create() and sem_close() functions.
 */
...
/* Define the semphore operation arrays for the semop() calls.
 */
...
static struct sembuf op_endcreate[2] = {
    {1, -1, SEM_UNDO},/* decrement [1] (proc counter) with undo on exit */
                      /* UNDO to adjust proc counter if process exits before explicitly
                       * calling sem_close() 
                       */
...
again:
     ...
     * The semflg specified both IPC_CREAT and IPC_EXCL and a semaphore set 
     ...
     * then increment it.
     * 
     * There is a race condition here. There is a possibility that between the semget()
     * above and the semop() below. another process can call our sem_close() function 
     * which can remove the semaphore if that process is the last one using it.
     * Therefore, we handle the error condition of an invalid semaphere ID specially below,
     * and if it does happens, we just go back and creat it begain.
     *
     */
    ...
    if (semval == 0) {
        /* 
         * We should initialize by doing a SETALL, but that would clear the adjust
         * value that we set when we locked the semaphore above. Instead, we'll do 
         * 2 system calls to initialize [0] and [1].
         */
    ...
    if (semop(id, &op_endcreate[0], 2) < 0) {
        err_sys("can't end op_endcreate");
    }
 ...
 * that the semphore must already exist. For example a client from a client-server
 * pair would use this, if its the server's responsibility to create the semaphore,
 * We return the semaphore ID if all OK, else -1.
 */
int
sem_open(key_t key)
 ...
 * When it is being down, as we do an TPC_RMID on the semaphore,
...
/* Close a semaphore
 * Unlike the remove function above, this function is for a process to call 
 * before it exits, when it is done with the semaphore.
 * We "decrement" the counter of process using the semaphore, and if this was the last one
 * we can remove the semaphore.
 */
void
sem_close(int id) 
{
...
/* 
 * Wait until a semaphore's value is greater than 0, then decrement it by 
 * 1 and return.
